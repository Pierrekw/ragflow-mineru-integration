...
class TaskCreateSchema(Schema):
    ...
    priority = fields.Str(missing='normal', validate=lambda x: x in [p.value for p in TaskPriority])
    data = fields.Dict(missing={})
    config = fields.Dict(missing={})
    queue_name = fields.Str(missing='default')
    scheduled_at = fields.DateTime(missing=None)
    max_retries = fields.Int(missing=3, validate=lambda x: 0 <= x <= 10)
    timeout_seconds = fields.Int(missing=3600, validate=lambda x: x > 0)
    dependencies = fields.List(fields.Str(), missing=[])
...
class TaskSearchSchema(Schema):
    ...
    task_type = fields.Str(missing=None, validate=lambda x: x in [t.value for t in TaskType] if x else True)
    status = fields.Str(missing=None, validate=lambda x: x in [s.value for s in TaskStatus] if x else True)
    priority = fields.Str(missing=None, validate=lambda x: x in [p.value for p in TaskPriority] if x else True)
    queue_name = fields.Str(missing=None)
    created_by = fields.Str(missing=None)
    assigned_to = fields.Str(missing=None)
    tags = fields.List(fields.Str(), missing=[])
    created_after = fields.DateTime(missing=None)
    created_before = fields.DateTime(missing=None)
    scheduled_after = fields.DateTime(missing=None)
    scheduled_before = fields.DateTime(missing=None)
    page = fields.Int(missing=1, validate=lambda x: x > 0)
    per_page = fields.Int(missing=20, validate=lambda x: 1 <= x <= 100)
    sort_by = fields.Str(missing='created_at', validate=lambda x: x in ['created_at', 'updated_at', 'scheduled_at', 'priority'])
    sort_order = fields.Str(missing='desc', validate=lambda x: x in ['asc', 'desc'])


class TaskUpdateSchema(Schema):
    """Task update validation schema."""
    name = fields.Str(missing=None, validate=lambda x: len(x) <= 255 if x else True)
    description = fields.Str(missing=None, validate=lambda x: len(x) <= 1000 if x else True)
    priority = fields.Str(missing=None, validate=lambda x: x in [p.value for p in TaskPriority] if x else True)
    assigned_to = fields.Str(missing=None)
    scheduled_at = fields.DateTime(missing=None)
